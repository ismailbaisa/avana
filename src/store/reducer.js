...
const reducer = (state = initialState, action) => {
    if (action.type === 'TOGGLER') {
        ...
        if (action.payload.show) {
            return {
                ...state, 
                DataSidebar: state.DataSidebar.map(
                    (data, i) => data.id === action.payload.id ? {...data, isShowed: false}
                                            : data
                )
            }
        }else{
            return {
                ...state, 
                DataSidebar: state.DataSidebar.map(
                    (data, i) => data.id === action.payload.id ? {...data, isShowed: true}
                                            : data
                )
            }

        }
    } else if (action.type === 'ALLOWER') {
        if (action.payload.show) {
            return {
                ...state, 
                DataSidebar: state.DataSidebar.map(
                    (data, i) => data.id === action.payload.id ? {...data, isAllowed: false}
                                            : data
                )
            }
        }else{
            return {
                ...state, 
                DataSidebar: state.DataSidebar.map(
                    (data, i) => data.id === action.payload.id ? {...data, isAllowed: true}
                                            : data
                )
            }
